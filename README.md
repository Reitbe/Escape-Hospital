...
```
- 특정 조건에서 벽을 들어 올려 이동시키는 기능을 구현했습니다. 벽을 들어 올리는 과정과 고정하는 과정으로 구분됩니다.
- 벽이 플레이어의 시야와 함께 움직일 수 있도록 기존 부모 오브젝트에서 벗어나 일시적으로 플레이어의 하위 오브젝트로 설정됩니다.
...
- 상태 및 오브젝트 전환 여부를 확인하는 이유는 정보의 변경이 필요한 때만 정보 제어 클래스의 함수를 호출하기 위함입니다.
- 이동 가능한 벽은 태그가 지정된 빈 오브젝트의 하위에 배치되어 있기 때문에 부모 오브젝트 확인 과정이 포함됩니다.
